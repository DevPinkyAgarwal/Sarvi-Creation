...
echo [*] Checking for existing processes on Port 5003...
for /f "tokens=5" %%a in ('netstat -ano ^| findstr :5003 ^| findstr LISTENING') do (
    echo [!] Found existing process %%a on Port 5003. Terminating...
    taskkill /PID %%a /F
)

echo [*] Starting Backend Server on Port 5003...
start "Sarvi Backend" cmd /k "cd backend && title Sarvi Backend && echo Starting Backend on Port 5003... && npm run dev"
